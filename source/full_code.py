...
p= subprocess.Popen(["alpr","-c","in","-j",path], shell=True, stdout=subprocess.PIPE)
...
cv2.waitKey()
cv2.imwrite("C:/full_code/testing/binary.jpg",binary_temp)
binary_temp=cv2.imread("C:/full_code/testing/binary.jpg")
...
for i in arr:
    x,y,w,h = cv2.boundingRect(contours[i])
    img_path="C:/full_code/testing/char-"+str(k)+".jpg"
    im=binary_temp[y:y+h,x:x+w]
    cv2.imwrite(img_path,im)
    TrainAndTest.prg(img_path)
    k=k+1
    cv2.rectangle(binary_temp,(x,y),(x+w,y+h),(0,255,0),1)
